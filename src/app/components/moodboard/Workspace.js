        ...
        <div className="w-full md:w-2/3 bg-white p-4 rounded-lg shadow-md flex flex-col">
            ...
            <div className="flex justify-between items-right mb-4">
                <div className="flex gap-2">
                    <button onClick={onSave} data-testid="save-collage" className="cursor-pointer bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700">
                        Save collage
                    </button>
                    <button onClick={onClear} data-testid="clear-workspace-button" className="cursor-pointer bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600">
                        Clear
                    </button>
                </div>
            </div>

            <div data-testid="workspace" className="grid grid-cols-3 gap-2 p-2 bg-gray-200 rounded-lg w-full min-h-[300px]" ref={collageRef}>
                {images.map((img, index) => (
                    ...
                    >
                        ...
                        <button onClick={() => onRemove(index)} data-testid="remove-workspace-image" className="absolute cursor-pointer top-1 right-1 bg-red-500 text-white rounded-full px-2 text-sm opacity-0 group-hover:opacity-100">
                            ✕
                        </button>
                    </div>
                ))}
